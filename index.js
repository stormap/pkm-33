...
async function initGame() {
    ...
    if (!FORCE_USE_DEFAULT_DATA && typeof globalBattleData !== 'undefined' && globalBattleData) {
        ...
        console.log('[PKM] Sử dụng dữ liệu được đưa vào từ bên ngoài (globalBattleData)');
    } else {
    ...
    // 加载对战 JSON (已在预加载阶段获取)
    try {
        console.log('[PKM] Sử dụng dữ liệu chiến đấu:', json);
        ...
        if (json.player && json.player.party) {
            ...
            console.log('[UNLOCK] Trạng thái mở khóa người chơi:', battle.playerUnlocks);
            ...
            if (json.player.trainerProficiency !== undefined) {
                ...
                console.log(`[COMMANDER] Đọc thông tin về năng lực của người huấn luyện từ định dạng JSON.: ${battle.trainerProficiency} (上限: ${proficiencyCap})`);
            }
            ...
            log(`<b>${battle.playerName}</b> Chuẩn bị cho trận chiến！`);
            ...
            if (typeof checkAndProcessNecrozmaFusion === 'function') {
                checkAndProcessNecrozmaFusion(battle.playerParty, log, () => {
                    console.log('[NECROZMA FUSION] Đã hoàn thành quá trình kiểm tra kết hợp.');
                });
            }
        } else {
            // Fallback: 默认玩家队伍
        ...
        if (t) {
            ...
            if (isWild) {
                log(`Pokémon hoang dã 【${battle.getEnemy().cnName}】 xuất hiện！`);
            } else {
        ...
        log(`Đối phương đã tung ra <b>${battle.getEnemy().cnName}</b> (Lv.${battle.getEnemy().level})!`);
        
        if (battle.scriptedResult === 'loss') {
            log(`<span style="color:#e67e22">[[Trận đấu cốt truyện] Đây là một trận đấu không thể thắng...</span>`);
        }
    } catch (e) {
    ...
    if (openingPoke) {
        log(`Đi thôi!！${openingPoke.cnName}（Lv.${openingPoke.level}）！`);
    }
    ...
    if (checkInitTransformFunc) {
        ...
        if (openingPoke && openingPoke.needsInitTransform) {
            ...
            if (result) {
                log(`<span style="color:#a855f7">✦ ${result.oldName} Trở thành ${result.newName}！</span>`);
                // 预加载新形态的精灵图，避免闪烁
        ...
        if (openingEnemy && openingEnemy.needsInitTransform) {
            ...
            if (result) {
                log(`<span style="color:#ef4444">✦ Đối phương ${result.oldName} Trở thành ${result.newName}！</span>`);
                // 预加载新形态的精灵图，避免闪烁
    ...
    if (json.environment && json.environment.weather && json.environment.weather !== 'none') {
        ...
        if (suppressionTier === 2) {
            tierHint = ' <span style="color:#f59e0b">[Vùng ức chế]</span>';
        } else if (suppressionTier === 3) {
            tierHint = ' <span style="color:#dc2626">[Tuyệt đối Lĩnh vực]</span>';
        }
    ...
    if (enableEnv && json.environment && json.environment.overlay) {
        ...
        if (typeof window.injectEnvironmentOverlay === 'function') {
            ...
            if (env) {
                ...
                console.log(`[ENV OVERLAY] Khởi tạo lớp môi trường: ${env.env_name}, số quy tắc: ${env.rules?.length || 0}`);
            }
        }
    }
    ...
    if (p.currHp <= 0) {
            ...
            if (i < p.moves.length) {
                ...
                if (p.volatile && p.volatile.disable > 0 && p.volatile.disabledMove) {
                    if (m.name === p.volatile.disabledMove) {
                        isDisabled = true;
                        console.log(`[DISABLE UI] ${m.name} bị phong ấn, nút bấm đã bị vô hiệu hóa`);
                    }
                }
                ...
                if (p.volatile && p.volatile.grudgeSealed && p.volatile.grudgeSealed.includes(m.name)) {
                    isDisabled = true;
                    console.log(`[GRUDGE UI] ${m.name} bị Grudge phong ấn, nút bấm đã bị vô hiệu hóa`);
                }
                
                // 【PP系统】PP 耗尽时禁用
                if (m.pp !== undefined && m.pp <= 0) {
                    isDisabled = true;
                    console.log(`[PP UI] ${m.name} cạn kiệt PP, nút bấm đã bị vô hiệu hóa`);
                }
                ...
                if (typeof window.envOverlay !== 'undefined' && window.envOverlay.isMoveBanned) {
                    if (window.envOverlay.isMoveBanned(p, m)) {
                        isDisabled = true;
                        envBanned = true;
                        console.log(`[ENV BAN UI] ${m.name} bị môi trường vô hiệu hóa, nút bấm chuyển sang màu xám`);
                    }
                }
        ...
        if (allDisabled && p.moves.length > 0) {
            ...
            if (struggleBtn) {
                ...
                console.log('[ENV BAN] Tất cả chiêu thức bị vô hiệu hóa, kích hoạt Struggle');
            }
        }
    }
